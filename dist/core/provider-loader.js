...
export class ProviderLoader {
     ...
     * 단일 TOON 파일 로드 및 파싱
     */
    async loadProviderFile(filePath) {
    ...
    async loadAllProviders() {
        ...
        const providers = await Promise.all(files.map(file => this.loadProviderFile(file)));
        return providers;
    }
    /**
     * TOON 파싱 결과를 ProviderSpec으로 변환
     */
    convertToProviderSpec(parsed, fallbackId) {
        // Provider 메타데이터 추출
        ...
        for (const [key, value] of Object.entries(parsed)) {
            if (key.startsWith('endpoints.')) {
                ...
                // 그룹 메타데이터 찾기
                const groupMeta = parsed.endpointGroups?.find((g) => g.groupId === groupName);
                endpointGroups[groupName] = {
                    ...
                    endpoints: this.convertEndpoints(value),
                };
            }
        }
        ...
        // Tools
        if (parsed.tools) {
            spec.tools = parsed.tools.map((tool) => ({
                name: tool.name,
                description: tool.description,
                category: tool.category || 'general',
                inputSchema: {}, // 추후 구현
                usesEndpoints: tool.usesEndpoints?.split(',').map((s) => s.trim()) || [],
            }));
        }
        return spec;
    }
    ...
    convertEndpoints(data) {
        ...
        return data.map(ep => ({
            ...
            schoolTypes: ep.schoolTypes?.split(',').map((s) => s.trim()),
