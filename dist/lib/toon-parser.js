...
export class ToonParser {
     ...
     * "Laptop 15\"",999,true
     */
    parseDataLine(line, schema) {
        const values = this.parseCsvLine(line);
        if (values.length !== schema.fields.length) {
            throw new Error(`Field count mismatch. Expected ${schema.fields.length} fields ` +
     ...
     * 행 개수 검증
     */
    validateCount(schema, data, _lineNumber) {
        if (!this.options.strictCount || !schema.count) {
            return;
        }
        if (data.length !== schema.count) {
            throw new Error(`Row count mismatch for '${schema.name}'. ` +
     ...
     */
    static fromJSON(json, tableName = 'data') {
 ...
 * TOON 파일 로드 및 파싱
 */
export async function loadToonFile(filePath, options) {
 ...
 * 객체를 TOON 파일로 저장
 */
export async function saveToonFile(filePath, data, tableName = 'data') {
    const fs = await import('fs/promises');
 ...
 */
export function multiTableToToon(tables) {
    const toonBlocks = [];
...
// 타입 가드
// ============================================
export function isToonSchema(line) {
    return /^[a-zA-Z0-9._-]+(\[\d+\])?\{[^}]+\}:$/.test(line.trim());
}
export function isToonComment(line, commentChar = '#') {
 ...
 * });
 */
//# sourceMappingURL=toon-parser.js.map